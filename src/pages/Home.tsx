...
import { Trophy, Users, Swords, Layers, ArrowRight, Award } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div>
      ...
      {/* PRIZE POOL */}
      <section className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-fuchsia-950/40 via-black to-red-950/30 p-8 sm:p-12">
          <Award className="absolute -right-4 -top-4 text-white/5" size={180} />
          <span className="mb-3 inline-block rounded-full border border-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-zinc-300">
            Призовой фонд
          </span>
          <h2 className="font-display max-w-xl text-3xl font-bold text-white sm:text-4xl">Награда чемпионам NJDC 2026</h2>
          <p className="mt-4 max-w-xl text-zinc-400">{settings.prize_text}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <span className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-zinc-200">
              🏆 CYBERSHOKE Lite ×2 игрока
            </span>
            <span className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-zinc-200">
              📜 Официальный диплом победителя
            </span>
          </div>
        </div>
      </section>
